...
{	
	...
	$license_request_url = "https://actify.com/license_requestor/sf3.php?partner_id=" . rawurlencode($partner_id);
	$message ="PARTNER_ID = " . $partner_id . "<BR>";
	$message .= "STARTING LICENSE REQUEST URL = " . $license_request_url . "<BR>";
	error_log( $message );
	...
	$message = "MODIFIED LICENSE REQUEST URL BEING RETURNED = " . $license_request_url . "<BR>";
	error_log( $message );
...
function actify_do_spinfire_professional_trial_license_request_and_set_confirmation($confirmation, $form, $lead, $ajax)
{
	...
	$message = "LEAD TYPE = " . $lead['9']. "<BR>";
	error_log( $message );
	if ($lead['9'] == 'Download')
	{
		// Do a license request
	$message = "IS DOWNLOAD. CALLING actify_get_trial_license_request_url <BR>";
	error_log( $message );
	...
	$message = "RECEIVED LICENSE REQUEST URL = " . $license_request_url . "<BR>";
	error_log( $message );

		$result_string = file_get_contents($license_request_url);
		
	$message = "RESULT STRING = " . $result_string . "<BR>";
	error_log( $message );
	...
	$message = "GETTING LICENSE TO SEND IN EMAIL USING RESULT STRING = " . $result_string . "<BR>";
	error_log( $message );
	...
	$message = "CALLING actify_send_license_email WITH PARAMS = " . "LEAD ID = " . $lead_id . " LEAD['6'] = " .  $lead['6'] . " SUBJECT = " . $subject . " BODY = " . $body . "<BR>";
	error_log( $message );
			if (actify_send_license_email($lead_id, $lead['6'], $subject, $body) == 1 )
				...
				// Display Message that there is a form erro
				$confirmation = __('We apologize for the inconvenience but we are currently having technical issues with our download forms. If you would like to request a trial of SpinFire please contact us at sales@actify.com.', 'actify');
		}
	}
	// CD
	else
	{
		...
		$confirmation = __('Thank you for your request for a copy of our SpinFire Professional Trial. Your request is being processed and you will receive an CD in the mail shortly.', 'actify');
	}
