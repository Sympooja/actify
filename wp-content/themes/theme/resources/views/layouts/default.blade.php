...
<body
    ...
    if (is_page('2182')) { // Replace 'your-page-slug' with your page's slug or ID
        if (isset($_POST['custom_password']) && $_POST['custom_password'] == 'abc@') {
            // Agar password sahi hai, toh kuch nahi karna (page ka content show hoga)
        } else {
            if (!isset($_POST['custom_password']) || $_POST['custom_password'] != 'abc@') {
                // Agar password galat hai, toh form dikhao
                echo '<form method="post" style="margin: 80px 0px 20; text-align: center;">
                        <p style="font-family: sans-serif;">This content is password protected. To view it please enter your password below:</p>
                        <label>Password:</label>
                        <input type="password" name="custom_password" required style="padding: 10px;"/>
                        <button type="submit" style="border: none;">Enter</button>
                      </form>';
                exit; // Page ka content load hone se rok dein
            }
        }
    }
}
add_action('template_redirect', 'custom_password_protection');
    ...
    @if (post_password_required())
     ...
     <script>
            // Clear the cache before showing the form
            function clearCacheBeforeForm() {
                if (typeof wp_cache_clear_cache === 'function') {
                    wp_cache_clear_cache(); // Clear WordPress cache
                }
            }

            // Call the cache clearing function when the page loads
            window.onload = function() {
                clearCacheBeforeForm(); // Clear cache when the page loads
            };
        </script>
    ...
    <script>
            ...
            // Function to clear WP Engine cache and reload the page after form submission
            function clearCacheAndReload() {
                // Clear cookies
                clearCookies();

                // Purge WP Engine cache
                if (typeof wpe_purge_cache === 'function') {
                    wpe_purge_cache(); // Purge WP Engine cache for the current page
                }

                // Reload the page after clearing cache and cookies
                location.reload(true); // Force a reload from the server (not from cache)
            }

            // When the page loads, check if the password form was submitted
            window.onload = function() {
                // If the password form was successfully submitted
                if (document.querySelector('form.post-password-form input[name="post_password"]').value) {
